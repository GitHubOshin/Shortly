function ShortenLink(props) {
  const { shortenLink, index } = props
  return (
    <div
      key={index}
      className="mt-10 bg-white mobile:w-[336.98px] desktop:w-[1110px] mobile:h-[155px] desktop:h-[72px] rounded-md flex mobile:flex-col desktop:flex-row mobile:items-start desktop:px-8 desktop:items-center mobile:justify-center desktop:justify-between"
    >
      ...
      <div className="flex mobile:px-4 desktop:px-0 w-full desktop:w-fit mobile:flex-col desktop:flex-row mobile:gap-2 desktop:gap-[25px]">
        <a
          href={shortenLink.newLink}
          className="text-cyan h-10 flex items-center"
        >
          {shortenLink.newLink}
        </a>
